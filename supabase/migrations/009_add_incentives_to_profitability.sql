-- Add Walmart funded incentives to order_profitability view
-- walmart_funded_incentive fees represent money Walmart pays the seller,
-- so they should be ADDED to revenue, not counted as costs.
-- This migration:
-- 1. Excludes walmart_funded_incentive from total_fees
-- 2. Adds a new walmart_incentive column
-- 3. Adjusts gross_profit = revenue - cogs - fees - shipping + incentives
...
SELECT
    ...
    COALESCE(incentives.walmart_incentive, 0) AS walmart_incentive,
    o.total
        - COALESCE(item_costs.cogs, 0)
        - COALESCE(fee_totals.total_fees, 0)
        - COALESCE(ship_costs.shipping_cost, 0)
        + COALESCE(incentives.walmart_incentive, 0)
        AS gross_profit,
...
LEFT JOIN LATERAL (
    ...
    WHERE cf.order_id = o.id
      AND cf.fee_type != 'walmart_funded_incentive'
) fee_totals ON true
LEFT JOIN LATERAL (
    SELECT COALESCE(SUM(cf.amount), 0) AS walmart_incentive
    FROM channel_fees cf
    WHERE cf.order_id = o.id
      AND cf.fee_type = 'walmart_funded_incentive'
) incentives ON true
